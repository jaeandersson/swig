...
static File *f_stubs_py = 0;          /* .pyi output file for -stubs */
static String *f_stubs = 0;           /* current receiving buffer (module-level or per-class body) */
static String *f_stubs_module = 0;    /* module-level buffer (outside any class) */
static String *f_stubs_class_body = 0;/* per-class body buffer (inside classHandler) */
static int f_stubs_class_members = 0; /* count of members emitted in current class */
...
static int nogil = 0;
static int stubs = 0;                 /* Emit PEP-484 .pyi stubs alongside the .py shadow */
static String *empty_string = 0;      /* reusable empty indent for stubEmitFunction */
static Hash *stub_alias_in_table = 0; /* typemap doc token -> PEP-484 expansion (input pos) */
static Hash *stub_alias_out_table = 0;/* typemap doc token -> PEP-484 expansion (output pos) */
...
static const char *usage3 = "\
     ...
     -stubs          - Also emit PEP-484 type stubs (.pyi) derived from typemap doc= annotations\n\
...
public:
  ...
  virtual void main(int argc, char *argv[]) {
	...
	if (strcmp(argv[i], "-interface") == 0) {
	   ...
	   * stub-related directives for backward compat with un-patched SWIG. */
  ...
  virtual int top(Node *n) {
	...
	Exit(EXIT_FAILURE);
      ...
      if (stubs) {
        if (!empty_string) empty_string = NewString("");
        String *stubname = NewStringf("%s%s.pyi", SWIG_output_directory(), Char(mainmodule));
        /* Also register a preamble section for user-supplied imports. */
        String *f_stubs_preamble = NewString("");
        Swig_register_filebyname("stubs_preamble", f_stubs_preamble);
        Setattr(n, "python:f_stubs_preamble", f_stubs_preamble);
        /* Alias tables populated via %widen_in / %widen_out, which
         * emit one `KEY=VALUE\n` line per call.  Parsed lazily on
         * first stubType lookup. */
        if (!stub_alias_in_table) stub_alias_in_table = NewHash();
        if (!stub_alias_out_table) stub_alias_out_table = NewHash();
        String *f_in = NewString("");
        String *f_out = NewString("");
        Swig_register_filebyname("stubs_alias_in_table", f_in);
        Swig_register_filebyname("stubs_alias_out_table", f_out);
        Setattr(n, "python:f_stubs_alias_in", f_in);
        Setattr(n, "python:f_stubs_alias_out", f_out);
        if ((f_stubs_py = NewFile(stubname, "w", SWIG_output_files())) == 0) {
          FileErrorDisplay(stubname);
          Exit(EXIT_FAILURE);
        }
        Delete(stubname);
  ...
  ------------------------------------------------*/\n", module);
    ...
    if (stubs && f_stubs_py) {
      /* Default preamble.  User-supplied imports via
       *   %insert("stubs_preamble") %{ import numpy as np %}
      ...
      Printv(f_stubs_py, "\n", NIL);
      if (Len(f_stubs_module) > 0)
        Printv(f_stubs_py, f_stubs_module, NIL);

      /* Emit __all__ listing every top-level public name.  Without this,
       * pyright refuses to treat `from .casadi import *` in __init__.pyi
       * as a full re-export, and inherited methods on re-exported classes
       * resolve as "Unknown" at use sites (e.g. MX.sym).  Scan the buffer
       * for column-0 definitions: `class NAME`, `def NAME`, and
       * module-level `NAME: TYPE` assignments. */
      Hash *seen = NewHash();
      List *names = NewList();
      const char *c = Char(f_stubs_module);
      int totlen = Len(f_stubs_module);
      int pos = 0;
      while (pos < totlen) {
        /* Only consider lines whose first char is column 0 (not indented). */
        bool at_col0 = (pos == 0 || c[pos - 1] == '\n');
        int end = pos;
        while (end < totlen && c[end] != '\n') end++;
        if (at_col0) {
          const char *line = c + pos;
          int llen = end - pos;
          const char *name = 0;
          int nlen = 0;
          if (llen > 6 && strncmp(line, "class ", 6) == 0) {
            name = line + 6;
            int k = 0;
            while (k < llen - 6 && (isalnum((unsigned char)name[k]) || name[k] == '_')) k++;
            nlen = k;
          } else if (llen > 4 && strncmp(line, "def ", 4) == 0) {
            name = line + 4;
            int k = 0;
            while (k < llen - 4 && (isalnum((unsigned char)name[k]) || name[k] == '_')) k++;
            nlen = k;
          } else if (llen > 1 && (isalpha((unsigned char)line[0]) || line[0] == '_')) {
            /* Module-level assignment: NAME: TYPE  or  NAME = VALUE */
            int k = 0;
            while (k < llen && (isalnum((unsigned char)line[k]) || line[k] == '_')) k++;
            if (k > 0 && k < llen && (line[k] == ':' || line[k] == '=' || line[k] == ' ')) {
              /* Reject reserved words / decorators (start with lowercase letters we recognize) */
              if (!(k == 4 && strncmp(line, "from", 4) == 0) &&
                  !(k == 6 && strncmp(line, "import", 6) == 0) &&
                  !(k == 3 && strncmp(line, "def", 3) == 0)) {
                name = line;
                nlen = k;
              }
            }
          }
          if (name && nlen > 0 && name[0] != '_') {
            String *nm = NewStringWithSize(name, nlen);
            if (!Getattr(seen, nm)) {
              Setattr(seen, nm, "1");
              Append(names, nm);
            }
      ...
      if (Len(names) > 0) {
        ...
        for (int i = 0; i < Len(names); ++i) {
          String *nm = (String *)Getitem(names, i);
          Printv(f_stubs_py, "    \"", nm, "\",\n", NIL);
        }
        Printv(f_stubs_py, "]\n", NIL);
      }
      Delete(names);
  ...
  /* ============================================================
   * PEP-484 .pyi stub emission (-stubs flag)
   *
   * Renders Python-style type tokens extracted from typemap doc=
   * annotations (as populated by the customdocs feature) into
   * PEP-484 type annotations and emits a .pyi sibling of the
   * shadow module file.
   *
   * Token alphabet expected (configured by the .i file):
   *   primitives:  int, bool, float, str
   *   lists:       [T]           ->  Sequence[T] (input)  / list[T] (output)
   *   maps:        dict:T        ->  Mapping[str, T]      / dict[str, T]
   *   pairs:       (T1,T2)       ->  tuple[T1, T2]
   *   classes:     passed through verbatim (a .pyi permits forward
   *                references everywhere, unquoted)
   *   fallback:    anything containing '<', '::' or '*' -> Any
   * ------------------------------------------------------------ */

  /* Resolve a pystub annotation.  Performs token-level alias
   * substitution — each identifier token in the pystub expression
   * is looked up in the alias table and, if present, replaced by
   * the alias body.  This lets `pystub_in="Sequence[DM]"` widen
   * the inner `DM` to the full `%stub_alias_in(DM, ...)` union
   * while preserving surrounding structure.
   *
   * No ad-hoc dict:T / [T] / (T,U) parsing — structure is carried
   * verbatim from the caller. */
  String *resolveStub(const String *tok, bool is_output) {
    if (!tok || Len(tok) == 0) return NewString("Any");
    stubDrainAliasBuffer(stub_alias_in_table,  "stubs_alias_in_table");
    stubDrainAliasBuffer(stub_alias_out_table, "stubs_alias_out_table");
    Hash *table = is_output ? stub_alias_out_table : stub_alias_in_table;
    /* First pass: whole-string alias lookup.  Needed because alias
     * keys can contain brackets (e.g. "[float]"). */
    if (table) {
      String *alias = Getattr(table, tok);
      if (alias && Len(alias) > 0) return Copy(alias);
    }
    if (!table) return Copy(tok);
    /* Second pass: walk tok, substitute each identifier token by
     * its alias body if present.  An identifier starts with letter
     * or underscore and contains word characters. */
    String *out = NewString("");
    const char *s = Char((String *)tok);
    int n = Len(tok);
    int i = 0;
    while (i < n) {
      if (isalpha((unsigned char)s[i]) || s[i] == '_') {
        ...
        String *alias = Getattr(table, ident);
        if (alias && Len(alias) > 0) {
          Printv(out, alias, NIL);
        } else {
  ...
  /* Look up the PEP-484 stub type for a parameter.  Reads the
   * explicit `pystub_in=` / `pystub_out=` typemap attribute; the
   * caller is responsible for ensuring every typemap carries pystub.
   * `tmap:in:doc` is a MATLAB/Python help-string label, not a type
   * expression — we don't recycle it.
   *
   * Fallback for SWIG-builtin typemaps (e.g. the primitive `int` /
   * `double` typemaps defined outside casadi.i) goes through
   * stubType(doc) which does alias lookup + raw-junk filter.  This
   * keeps the tool usable outside casadi too. */
  String *pickStub(Parm *pj, bool is_output) {
    const char *attr = is_output ? "tmap:in:pystub_out" : "tmap:in:pystub_in";
    String *pystub = Getattr(pj, attr);
    if (pystub && Len(pystub) > 0)
      return resolveStub(pystub, is_output);
    String *pystub_any = Getattr(pj, "tmap:in:pystub");
    if (pystub_any && Len(pystub_any) > 0)
      return resolveStub(pystub_any, is_output);
    /* SWIG-builtin fallback */
    String *tdoc = Getattr(pj, "tmap:in:doc");
    return tdoc ? stubType(tdoc, is_output) : NewString("Any");
  }

  /* Forward-declare: recursive type token translation */
  String *stubType(const String *tok, bool is_output) {
    if (!tok || Len(tok) == 0)
    ...
    /* Look up user-provided aliases.  Sources:
     *   - %stub_alias_in / %stub_alias_out (emit a
     *     %insert("stubs_alias_*_table") line; drained here on demand).
     *   - legacy %pragma(python) stub_alias_{in,out}="TOK=..." (still
     *     supported via pragmaDirective but new code should prefer
     *     %stub_alias_in which also defines a SWIG token `TOK_IN` for
     *     use inside hand-written %stub_* invocations).
     * Widening is for input-position types that casadi's to_ptr<>
     * accepts beyond what the typemap doc string suggests. */
    stubDrainAliasBuffer(stub_alias_in_table,  "stubs_alias_in_table");
    stubDrainAliasBuffer(stub_alias_out_table, "stubs_alias_out_table");
    {
      Hash *table = is_output ? stub_alias_out_table : stub_alias_in_table;
      if (table) {
        String *key = NewStringWithSize(s, n);
        String *alias = Getattr(table, key);
        Delete(key);
        if (alias && Len(alias) > 0)
          return Copy(alias);
      }
    ...
    /* Raw C++ types sneak through when a function isn't covered by
     * a casadi_typemaps invocation.  Punt to Any. */
    for (int i = 0; i < n; ++i) {
      if (s[i] == '<' || s[i] == '*' || s[i] == '&')
        return NewString("Any");
      if (i + 1 < n && s[i] == ':' && s[i + 1] == ':')
    ...
    /* SwigType mangled pointer forms like "p.PyObject" or anything
     * containing a dot collapse to Any — these are opaque handles
     * at the Python level. */
    for (int i = 0; i < n; ++i) {
      if (s[i] == '.')
        return NewString("Any");
    ...
    /* Untyped PyObject handles (GUESTOBJECT typedef, PyObject
     * struct) — opaque at the stub level. */
    if ((n >= 8 && strncmp(s, "PyObject", 8) == 0) ||
        (n >= 10 && strncmp(s, "GUESTOBJECT", 10) == 0))
      return NewString("Any");

    /* C primitives that fell through without a typemap doc=. */
    if ((n == 6  && strncmp(s, "size_t",    6)  == 0) ||
        (n == 7  && strncmp(s, "ssize_t",   7)  == 0) ||
        (n == 9  && strncmp(s, "ptrdiff_t", 9)  == 0) ||
        (n == 4  && strncmp(s, "long",      4)  == 0) ||
        (n == 9  && strncmp(s, "long long", 9)  == 0) ||
        (n == 5  && strncmp(s, "short",     5)  == 0) ||
        (n == 8  && strncmp(s, "unsigned",  8)  == 0))
      return NewString("int");
    if ((n == 6 && strncmp(s, "double", 6) == 0))
      return NewString("float");

    /* C++ keyword 'char' appears for strings on MATLAB; on Python our
     * L_STR macro is already "str", but guard anyway. */
    if (n == 4 && strncmp(s, "char", 4) == 0)
      return NewString("str");

    /* Primitive and class names pass through verbatim.  The historical
     * `dict:T` / `[T]` / `(T,U)` parsing branches were retired in
     * favour of the `pystub_in=` / `pystub_out=` typemap attributes
     * authored at the %casadi_template / %casadi_typemaps call site —
     * SWIG typemap doc= strings are a display label, not a Python
     * type expression. */
    return NewStringWithSize(s, n);
  }

  /* Python literal for a parameter's default value, or NIL when it is
   * not a simple bool/int/float/None (typeshed style: those get `...`). */
  String *stubDefaultLiteral(Parm *p) {
    String *stringval = Getattr(p, "stringval");
    if (stringval) return NIL;
    String *result = convertValue(Getattr(p, "value"), Getattr(p, "numval"), stringval, Getattr(p, "type"));
    if (!result) return NIL;
    /* convertValue also yields enum item names; those may live in a
     * scope the stub cannot see from here, so keep literals only. */
    const char *c = Char(result);
    bool literal = Equal(result, "True") || Equal(result, "False") || Equal(result, "None") ||
                   (*c && (isdigit((unsigned char)*c) || *c == '-' || *c == '+' || *c == '.'));
    if (!literal) { Delete(result); return NIL; }
    return result;
  }

  /* Emit a single PEP-484 signature line (no newline).  Overload
   * dispatch is handled by the caller.
   *
   * Walks wrap:parms directly (mirroring the same traversal that
   * populate_docParmList uses) so we can read each parm's default
   * value alongside its typemap doc string. */
  void stubWriteSignature(String *f, const String *indent, Node *ni,
                          bool needs_self, bool is_constructor) {
    String *name = is_constructor ? NewString("__init__") : Copy(Swig_symname(ni));

    /* Expand SWIG defaultargs alias if present — matches make_pyParmList. */
    Node *alias = Getattr(ni, "defaultargs");
    Node *source = alias ? alias : ni;

    Printv(f, indent, "def ", name, "(", NIL);
    bool first = true;
    if (needs_self || is_constructor) {
      Printv(f, "self", NIL);
      first = false;
    }

    /* Collect input parms (via typemap doc) and output parms in two passes */
    List *out_types = NewList();

    /* A maybecall `__eq__`/`__ne__` returns NotImplemented on argument
     * mismatch, so per the data model its operand is `object`. */
    bool eq_object = GetFlag(ni, "feature:python:maybecall") &&
                     (Equal(name, "__eq__") || Equal(name, "__ne__"));
    ...
    while (pj) {
      ...
      if (is_input || is_argout) {
        String *pname = Getattr(pj, "name");
        if (!pname) pname = Getattr(pj, "lname");
        bool is_self = pname && Strcmp(pname, "self") == 0;

        if (is_argout && !is_input) {
          /* Pure output — contributes to return, not inputs.  Stored
           * as already-resolved stub type; the return-emission path
           * calls stubType on leftover legacy doc strings. */
          String *resolved = pickStub(pj, true);
          Append(out_types, resolved);
        } else if (!is_self) {
          String *rendered = eq_object ? NewString("object") : pickStub(pj, false);
          if (!first) Printv(f, ", ", NIL);
          Printv(f, pname ? Char(pname) : "_", ": ", rendered, NIL);
          String *pvalue = Getattr(pj, "value");
          if (pvalue && Len(pvalue) > 0) {
            String *lit = stubDefaultLiteral(pj);
            Printv(f, " = ", lit ? lit : "...", NIL);
            Delete(lit);
          }
          Delete(rendered);
          first = false;
    ...
    if (is_constructor) {
      ...
      /* Look up return type: prefer pystub_out / pystub attributes,
       * fall back to doc, finally synthesise from SwigType. */
      String *ret_stub = 0;
      if (!is_void) {
        String *po = Getattr(source, "tmap:out:pystub_out");
        if (!po || Len(po) == 0) po = Getattr(source, "tmap:out:pystub");
        if (po && Len(po) > 0) {
          ret_stub = resolveStub(po, true);
        } else {
          String *ret_doc = Getattr(source, "tmap:out:doc");
          String *ret_fallback = 0;
          if (!ret_doc) {
            SwigType *rt = Getattr(source, "type");
            if (rt) {
              String *sname = SwigType_namestr(rt);
              if (sname) {
                const char *cs = Char(sname);
                if (strncmp(cs, "casadi::", 8) == 0)
                  ret_fallback = NewString(cs + 8);
                else
                  ret_fallback = Copy(sname);
                Delete(sname);
              }
            }
            ret_doc = ret_fallback;
          }
          ret_stub = ret_doc ? stubType(ret_doc, true) : NewString("Any");
          if (ret_fallback) Delete(ret_fallback);
        }
      }
      int n_extra = Len(out_types);

      int total = (is_void ? 0 : 1) + n_extra;
      if (total == 0) {
        Printv(f, "None", NIL);
      } else if (total == 1) {
        if (!is_void) {
          Printv(f, ret_stub, NIL);
        } else {
          /* out_types items are already resolved stub strings */
          Printv(f, (String *)Getitem(out_types, 0), NIL);
        }
      } else {
        Printv(f, "tuple[", NIL);
        bool first_ret = true;
        ...
        for (int i = 0; i < n_extra; ++i) {
          ...
          Printv(f, (String *)Getitem(out_types, i), NIL);
          first_ret = false;
        }
        Printv(f, "]", NIL);
      }
      if (ret_stub) Delete(ret_stub);
    }
  ...
  /* Emit the full set of overload stubs for a function-like node.
   * Kind: 0=free function, 1=member method, 2=static method,
   * 3=constructor. */
  void stubEmitFunction(Node *n, const String *indent, int kind) {
    if (!stubs || !f_stubs) return;
    /* Honour %feature("python:stub_skip") — set this on a method to
     * suppress auto-emission of its .pyi overload chain, so the user
     * can replace it with hand-written %stub_method declarations
     * whose signatures reflect the Python-visible calling convention
     * (e.g. Callback.eval_buffer collapses the 4-arg C++ signature
     * into a 2-tuple Python interface). */
    if (GetFlag(n, "feature:python:stub_skip")) return;
    ...
    for (int i = 0; i < nfunc; ++i) {
      ...
      if (nfunc > 1)
        Printv(f_stubs, indent, "@overload\n", NIL);
      if (kind == 2)
        Printv(f_stubs, indent, "@staticmethod\n", NIL);
      bool needs_self = (kind == 1);
      bool is_ctor = (kind == 3);
      stubWriteSignature(f_stubs, indent, ni, needs_self, is_ctor);
    }
    if (kind != 0 /* free */ ) f_stubs_class_members++;
    if (single) Delete(single);
  }

  /* Drain the %insert("stubs_alias_{in,out}_table") buffer — each line
   * is one `TOK=PEP484` entry registered during SWIG parsing — into
   * the given hash.  Consumes the buffer so repeated invocation is
   * idempotent. */
  void stubDrainAliasBuffer(Hash *&table, const String *buffer_key) {
    if (!table) table = NewHash();
    File *f = Swig_filebyname(buffer_key);
    ...
    for (int i = 0; i <= n; ++i) {
      if (i == n || c[i] == '\n') {
        if (i > start) {
          String *line = NewStringWithSize(c + start, i - start);
          stubRecordAlias(table, line);
          Delete(line);
        }
        start = i + 1;
      }
    }
    Clear(s);  /* idempotent: after drain, buffer is empty */
  }

  /* Parse a "TOK=PEP484" value and register it in the given table. */
  void stubRecordAlias(Hash *&table, const String *value) {
    if (!table) table = NewHash();
    if (!value || Len(value) == 0) return;
    const char *s = Char((String *)value);
    int n = Len(value);
    int eq = 0;
    while (eq < n && s[eq] != '=') eq++;
    if (eq >= n) {
      Swig_warning(WARN_NONE, "<pragma>", 0,
                   "stub_alias pragma expects 'TOKEN=ANNOTATION', got: %s\n",
                   Char((String *)value));
      return;
    }
    int k_end = eq;
    while (k_end > 0 && (s[k_end - 1] == ' ' || s[k_end - 1] == '\t')) k_end--;
    int v_start = eq + 1;
    while (v_start < n && (s[v_start] == ' ' || s[v_start] == '\t')) v_start++;
    int v_end = n;
    while (v_end > v_start && (s[v_end - 1] == ' ' || s[v_end - 1] == '\t')) v_end--;
    String *key = NewStringWithSize(s, k_end);
    String *val = NewStringWithSize(s + v_start, v_end - v_start);
    Setattr(table, key, val);
    Delete(key); Delete(val);
  }

  /* Emit a module-level or class-level attribute annotation.
   * For variables we use the raw C type (via SwigType_str) only as
   * a last resort — we haven't got a doc= channel for variables. */
  void stubEmitVariable(Node *n, const String *indent) {
    if (!stubs || !f_stubs) return;
    ...
    if (pystub && Len(pystub) > 0) {
      Delete(rendered);
      rendered = resolveStub(pystub, true);
    } else if (doc && Len(doc)) {
      Delete(rendered);
  ...
  void dispatchFunction(Node *n, String *linkage, int funpack = 0, bool builtin_self = false, bool builtin_ctor = false, bool director_class = false, bool use_static_method = false) {
    ...
    if (stubs && !(shadow & PYSHADOW_MEMBER) && use_static_method && !in_class) {
      stubEmitFunction(n, empty_string, 0);
    }
    DelWrapper(f);
    Delete(dispatch);
  ...
  */
  ...
  virtual int functionWrapper(Node *n) {
    ...
    if (checkAttribute(n, "feature:customdoc", "1") && !GetFlag(n, "feature:python:maybecall")) {
  ...
  virtual int classHandler(Node *n) {
    ...
    /* Open per-class stub body buffer.  Also re-point the "stubs" named
     * file so that %insert("stubs") %{ ... %} blocks inside the class body
     * land in the per-class buffer instead of the module-level buffer. */
    String *f_stubs_class_body_saved = f_stubs_class_body;
    String *f_stubs_saved = f_stubs;
    int f_stubs_class_members_saved = f_stubs_class_members;
    if (stubs) {
      f_stubs_class_body = NewString("");
      f_stubs = f_stubs_class_body;
      f_stubs_class_members = 0;
      Swig_register_filebyname("stubs", f_stubs_class_body);
    }
    ...
    /* Flush per-class stub body to module stream */
    if (stubs) {
      /* Render class header: class Name(Bases): */
      Printv(f_stubs_module, "\nclass ", class_name, NIL);
      List *baselist = Getattr(n, "bases");
      bool printed_base = false;
      if (baselist && Len(baselist)) {
        Iterator b = First(baselist);
        while (b.item) {
          String *bname = Getattr(b.item, "python:proxy");
          ...
          if (bname && !ignore) {
            if (!printed_base) {
              Printv(f_stubs_module, "(", bname, NIL);
              printed_base = true;
            } else {
              Printv(f_stubs_module, ", ", bname, NIL);
            }
        ...
        if (printed_base) Printv(f_stubs_module, ")", NIL);
      }
      Printv(f_stubs_module, ":\n", NIL);

      if (f_stubs_class_body && Len(f_stubs_class_body) > 0) {
        Printv(f_stubs_module, f_stubs_class_body, NIL);
      } else {
        Printv(f_stubs_module, "    ...\n", NIL);
      }

      if (f_stubs_class_body) Delete(f_stubs_class_body);
      f_stubs_class_body = f_stubs_class_body_saved;
      f_stubs = f_stubs_saved;
      f_stubs_class_members = f_stubs_class_members_saved;
      /* Restore "stubs" named-file routing to the module-level buffer
       * (or to the enclosing class body, if classes happened to nest). */
      Swig_register_filebyname("stubs",
                               f_stubs_class_body ? f_stubs_class_body : f_stubs_module);
    }
   ...
   *
   * Recognised pragmas:
   *   %pragma(python) stub_alias_in = "TOK=PEP484 expansion"
   *   %pragma(python) stub_alias_out = "TOK=PEP484 expansion"
   * ------------------------------------------------------------ */
  ...
  virtual int pragmaDirective(Node *n) {
    if (!ImportMode && stubs) {
      ...
      String *value = Getattr(n, "value");
      if (lang && cmd && Strcmp(lang, "python") == 0) {
        if (Strcmp(cmd, "stub_alias_in") == 0) {
          stubRecordAlias(stub_alias_in_table, value);
        } else if (Strcmp(cmd, "stub_alias_out") == 0) {
          stubRecordAlias(stub_alias_out_table, value);
        }
      }
    }
  ...
  virtual int memberfunctionHandler(Node *n) {
	...
	Delete(fullname);
      ...
      /* Emit .pyi overloads for this member (once per overload chain) */
      if (stubs) stubEmitFunction(n, tab4, 1);
    }
    return SWIG_OK;
  }
  ...
  virtual int staticmemberfunctionHandler(Node *n) {
    ...
    /* Emit .pyi overloads for this static member */
    if (stubs) stubEmitFunction(n, tab4, 2);
    return SWIG_OK;
  }
  ...
  virtual int constructorHandler(Node *n) {
	...
	Delete(subfunc);
      ...
      /* Emit .pyi __init__ overloads (once per overload chain) */
      if (stubs) stubEmitFunction(n, tab4, 3);
    }
    return SWIG_OK;
  }
  ...
  virtual int membervariableHandler(Node *n) {
    ...
    if (stubs) stubEmitVariable(n, tab4);
    return SWIG_OK;
  }
