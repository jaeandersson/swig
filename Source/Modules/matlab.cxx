...
MATLAB::MATLAB():
  ...
  Printv(director_prot_ctor_code,
	 ...
	 "  $director_new \n",
	 "} else {\n", "   SWIG_Matlab_ErrorId(\"SWIG:RuntimeError\",\"accessing abstract class or protected constructor\"); \n", "  SWIG_fail;\n", "}\n", NIL);
...
int MATLAB::top(Node *n) {
  ...
  if (i.item) {
    ...
    Printf(f_init, "if (!id) SWIG_Matlab_ErrorId(\"SWIG:RuntimeError\", \"Setup failed\");\n");
    for (; i.item; i = Next(i)) {
      Printf(f_init, "error = SWIG_Matlab_CallInterp(0, 0, 1, &id, \"%s\");\n", i.item);
      Printf(f_init, "if (error) SWIG_Matlab_ErrorId(\"SWIG:RuntimeError\", \"Cannot initialize %s\");\n", i.item);
      Delete(i.item);
    }
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"This mex file should only be called from "
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"The function should have one output.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"No such constant ID.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  ...
  // Make sure found
  Printf(f_begin, "  if (s==0) {\n");
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"No such function ID.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"swigThis should have one input and one output.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"The argument should be a SWIG class.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  ...
  Printf(f_begin, "    SWIG_Error_Format(SWIG_RuntimeError, \"swigTouch should have no inputs or outputs.\");\n");
  Printf(f_begin, "    return 1;\n");
  Printf(f_begin, "  }\n");
  Printf(f_begin, "  return 0;\n");
  Printf(f_begin, "}\n\n");
...
int MATLAB::functionWrapper(Node *n) {
  ...
  if (customdoc) {
    ...
    Printf(f->code, "SWIG_Error_Format(SWIG_RuntimeError, \"No matching function for overload function '%s'.\"\n", Getattr(n, "sym:name"));
...
void MATLAB::finalizeGateway() {
  ...
  Printf(f_gateway, "    SWIG_Matlab_ErrorId(SWIG_ErrorType(SWIG_lasterror_code), SWIG_lasterror_msg);\n");
  Printf(f_gateway, "  }\n");
  Printf(f_gateway, "}\n");
}
...
void MATLAB::initConstant() {
  ...
  Printf(f_constants,
	 "    SWIG_Error_Format(SWIG_RuntimeError, \"This function should only be called from inside the .m files generated by SWIG. First input should be the constant ID .\");\n");
...
void MATLAB::finalizeConstant() {
  ...
  Printf(f_constants, "    SWIG_Error_Format(SWIG_RuntimeError, \"No such constant.\");\n");
  Printf(f_constants, "    return 1;\n");
  Printf(f_constants, "  }\n");
  if (num_constant == 0) {
