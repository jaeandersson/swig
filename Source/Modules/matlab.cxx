...
int MATLAB::top(Node *n) {
  ...
  */
  ...
  // Get swigPtr for this module
  Printf(f_begin,"\n");
  Printf(f_begin,"int swigModulePtr(int resc, mxArray *resv[], int argc, mxArray *argv[]) {\n");

  // The first argument is always the ID
  Printf(f_begin,"  if ( argc!=0 ) {\n");
  Printf(f_begin,"    mexWarnMsgIdAndTxt(\"SWIG:RuntimeError\", \"This function does not take any input arguments.\");\n");
...
void MATLAB::initGateway() {
  ...
  // swigPtr pointer retrieval function has index 3
  String* swigModulePtr=NewString("swigModulePtr");
  toGateway(swigModulePtr, swigModulePtr);
  Delete(swigModulePtr);
}
...
void MATLAB::wrapConstructor(int gw_ind, String *symname, String *fullname, Node *n) {
    ...
    if (fullname==0) {
      ...
      if (have_matlabprepend(n))
        Printf(f_wrap_m, "%s\n",matlabprepend(n));
      Printf(f_wrap_m,"        self.swigFcn = '%s';\n", mex_fcn);
